...
static plugin_interface_t inputtrace_interface;
...
FILE * inputtrace_log=DECAF_NULL_HANDLE;
...
void do_read_taint_mem(DECAF_Callback_Params *param)
{
	...
	if(VMI_locate_module_c(eip,cr3, name, &dm) == -1 && !inputtrace_log)
	{
		DECAF_printf("\n dm is null 0x%08x  \n",cr3);
		return;
	}
	...
	if(param->rt.size <=4)
		fprintf(inputtrace_log,"%s   \t 0 \t 0x%08x \t\t 0x%08x \t %d      0x%08x  0x%08x %15s    \t%s\t%s\n",
			name, param->rt.vaddr, param->rt.paddr, param->rt.size,*((uint32_t *)param->rt.taint_info), eip, dm.name,modname_t,func_name_t);
	else
		fprintf(inputtrace_log,"%s   \t 0 \t 0x%08x \t\t 0x%08x \t %d      0x%16x  0x%08x %15s     \t%s\t%s\n",
...
void do_write_taint_mem(DECAF_Callback_Params *param)
{
	...
	if(stack_top)
	{
		...
		{
			funcmap_get_name_c(sys_call_entry_stack[stack_top-1], cr3, modname_t, func_name_t);
			 fprintf(inputtrace_log,"\nSTACK_WRITE_TAINT_MEM \t sys_call_entry_stack:[ %x ] func_name:[ %s ]\n", sys_call_entry_stack[stack_top-1], func_name_t);
		}
		else {
			memset(modname_t, 0, 512);
			memset(func_name_t, 0, 512);
	...
	//fprintf(inputtrace_log,"%s    1  0x%08x  0x%08x  %d   0x%08x %s   0x%08x  %d    %s\n",
...
void do_taint_sendkey(Monitor *mon, const QDict *qdict)
{
  // Set the origin and offset for the callback
	...
	DECAF_printf("please input a key\n");
}

void inputtrace_cleanup()
{
	  if(inputtrace_log)
			fclose(inputtrace_log);
		...
		inputtrace_log = DECAF_NULL_HANDLE;
		handle_block_end_cb = DECAF_NULL_HANDLE;

}
void do_enable_inputtrace_check( Monitor *mon, const QDict *qdict)
{
	const char *tracefile_t = qdict_get_str(qdict, "tracefile");
	inputtrace_log= fopen(tracefile_t,"w");
	if(!inputtrace_log)
	{
		DECAF_printf("the %s can not be open or created !!",tracefile_t);
		return;
	}
	fprintf(inputtrace_log,"Process Read(0)/Write(1) vaddOfTaintedMem   paddrOfTaintedMem    Size   "
...
void do_disable_inputtrace_check( Monitor *mon, const QDict *qdict)
{
	inputtrace_cleanup();
	DECAF_printf("disable taintmodule check successfully \n");
}

static mon_cmd_t inputtrace_term_cmds[] = {
#include "plugin_cmds.h"
		{ NULL, NULL, }, };

plugin_interface_t* init_plugin(void) {
	inputtrace_interface.mon_cmds = inputtrace_term_cmds;
	inputtrace_interface.plugin_cleanup = &inputtrace_cleanup;

	//initialize the plugin

	return (&inputtrace_interface);
}
