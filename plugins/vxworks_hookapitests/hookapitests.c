...
static DECAF_Handle vxworks_handle = DECAF_NULL_HANDLE;
...
} vxworks_hook_context_t;

static void vxworks_ret(void *param)
{
	vxworks_hook_context_t *ctx = (vxworks_hook_context_t *)param;
	DECAF_printf("[%s] exit:", targetname);
...
static void vxworks_call(void *opaque)
{
	DECAF_printf("[%s] entry !\n", targetname);
	vxworks_hook_context_t *ctx = (vxworks_hook_context_t*)
			malloc(sizeof(vxworks_hook_context_t));
...
static void register_hooks()
{
	...
	vxworks_handle = hookapi_hook_function_byname(
			"vxworks_kernel", targetname, 1, targetcr3,
			vxworks_call, NULL, 0);
...
/*
	...
	*/
	...
	//DECAF_printf("Function hook: %s, CR3: %x \n", targetname, targetcr3);
