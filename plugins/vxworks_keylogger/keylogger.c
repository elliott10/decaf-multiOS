...
void do_read_taint_mem(DECAF_Callback_Params *param)
{
	...
	if(VMI_locate_module_c(eip, ebp, name, &dm) == -1 && !keylogger_log)
	{
		DECAF_printf("\n dm is null 0x%08x  \n",ebp);
		return;
	}
	if(stack_top)
	{
		...
		if(ebp == cr3_stack[stack_top-1])
		{
			funcmap_get_name_c(sys_call_entry_stack[stack_top-1], ebp, modname_t, func_name_t);
  			printf("Read taint [%s] [%s] \n ",modname_t,func_name_t);
		}
		else {
