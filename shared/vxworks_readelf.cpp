...
#define vxworks_byte_get(field)		vxworks_byte_get (field, sizeof (field))
...
elf_vma
vxworks_byte_get (unsigned char *field, int size)
{
	switch (size)
	{
	case 1:
...
static void vxworks_register_symbol(const char * mod_name, const char * func_name,
...
vxworks_get_section_name (const Elf_Internal_Shdr& section, ELFInfo & elf) {
	if (!elf.string_table)
		return "<no-name>";
...
static bool
vxworks_process_section_header (ELFInfo & elf)
{
...
// get program header
static int
vxworks_get_program_headers (ELFInfo & elf)
{
	...
	for (size_t i = 0; i < elf.elf_header.e_phnum; i++)
	{
		...
		internal.p_type   = vxworks_byte_get (external[i].p_type);
		internal.p_offset = vxworks_byte_get (external[i].p_offset);
		internal.p_vaddr  = vxworks_byte_get (external[i].p_vaddr);
		internal.p_paddr  = vxworks_byte_get (external[i].p_paddr);
		internal.p_filesz = vxworks_byte_get (external[i].p_filesz);
		internal.p_memsz  = vxworks_byte_get (external[i].p_memsz);
		internal.p_flags  = vxworks_byte_get (external[i].p_flags);
		internal.p_align  = vxworks_byte_get (external[i].p_align);
...
static bool
vxworks_process_program_headers (ELFInfo & elf)
{
	if (elf.elf_header.e_phnum == 0 || !vxworks_get_program_headers (elf) )
		return false;
		...
		if (segment->p_type == PT_DYNAMIC) {
			...
			if (!elf.section_headers.empty()) {
				...
				for ( ; sec != elf.section_headers.end(); sec++) {
					if ( !vxworks_get_section_name(*sec, elf).compare(".dynamic") )
						break;
				}
...
static long
vxworks_offset_from_vma (ELFInfo & elf, bfd_vma vma, bfd_size_type size)
{
	if ( !vxworks_get_program_headers (elf))
	{
...
static bool
vxworks_read_elf_section_headers ( ELFInfo & elf, size_t readNum )
{
	...
	for (size_t i = 0; !readError && i < readNum; i++)
	{
		...
		internal.sh_name      = vxworks_byte_get (shdrs[i].sh_name);
		internal.sh_type      = vxworks_byte_get (shdrs[i].sh_type);
		internal.sh_flags     = vxworks_byte_get (shdrs[i].sh_flags);
		internal.sh_addr      = vxworks_byte_get (shdrs[i].sh_addr);
		internal.sh_offset    = vxworks_byte_get (shdrs[i].sh_offset);
		internal.sh_size      = vxworks_byte_get (shdrs[i].sh_size);
		internal.sh_link      = vxworks_byte_get (shdrs[i].sh_link);
		internal.sh_info      = vxworks_byte_get (shdrs[i].sh_info);
		internal.sh_addralign = vxworks_byte_get (shdrs[i].sh_addralign);
		internal.sh_entsize   = vxworks_byte_get (shdrs[i].sh_entsize);
...
static bool
vxworks_get_elf_symbols ( ELFInfo & elf, const Elf_Internal_Shdr & section, std::vector<Elf_Internal_Sym> & symbols )
{
	...
	for (size_t i = 0; i < number; i++)
	{
		...
		tmp.st_name  = vxworks_byte_get (esyms[i].st_name);
		tmp.st_value = vxworks_byte_get (esyms[i].st_value);
		tmp.st_size  = vxworks_byte_get (esyms[i].st_size);
		tmp.st_shndx = vxworks_byte_get (esyms[i].st_shndx);
		if (tmp.st_shndx == (SHN_XINDEX & 0xffff) && shndx != NULL)
			tmp.st_shndx
				= vxworks_byte_get ((unsigned char *) &shndx[i], sizeof (shndx[i]));
		else if (tmp.st_shndx >= (SHN_LORESERVE & 0xffff))
			tmp.st_shndx += SHN_LORESERVE - (SHN_LORESERVE & 0xffff);
		tmp.st_info  = vxworks_byte_get (esyms[i].st_info);
		tmp.st_other = vxworks_byte_get (esyms[i].st_other);
...
static bool
vxworks_process_section_headers ( ELFInfo & elf )
{
	...
	if ( !vxworks_read_elf_section_headers ( elf, elf.elf_header.e_shnum ) )
		return false;
	...
	for (section = elf.section_headers.begin(); section != elf.section_headers.end(); section++) {
		const std::string name = vxworks_get_section_name( *section, elf );
		if (section->sh_type == SHT_DYNSYM) {
			...
			if ( !vxworks_get_elf_symbols ( elf, *section, elf.dynamic_symbols ) )
				return false;
		}
...
get_dynamic_section (ELFInfo & elf)
{
	...
	for (size_t i = 0; i < section_count; i++) {
		...
		entry.d_tag = vxworks_byte_get ( (edyn+i)->d_tag );
		entry.d_un.d_val = vxworks_byte_get ( (edyn+i)->d_un.d_val );
...
process_dynamic_section ( ELFInfo & elf )
{
	...
	if ( elf.dynamic_symbols.empty() ) {
		for (std::vector<Elf_Internal_Dyn>::iterator it = elf.dynamic_section.begin();
			...
			section.sh_offset = vxworks_offset_from_vma (elf, it->d_un.d_val, 0);
	...
	if ( elf.dynamic_strings == NULL )
	{
		for (std::vector<Elf_Internal_Dyn>::iterator it = elf.dynamic_section.begin();
			...
			offset = vxworks_offset_from_vma (elf, it->d_un.d_val, 0);
...
static bool
vxworks_process_symbol_table ( ELFInfo  & elf )
{
			...
			section++)
	{
		...
		monitor_printf ( default_mon, "[%s] symbol table '%s' contains %lu entries: (size %u, entsize %u)\n",
				elf.mod_name, vxworks_get_section_name (*section, elf).c_str() ,
				(unsigned long) (section->sh_size / section->sh_entsize),
				section->sh_size, section->sh_entsize);
#endif

		// symtab will be cleaned inside the function
		if ( !vxworks_get_elf_symbols ( elf, *section, symtab ) )
			return false;
		if ( symtab.empty() )
			continue;
		...
		for ( std::vector<Elf_Internal_Sym>::iterator it = symtab.begin(); it != symtab.end(); it++ )
		{
			...
			if ( ELF_ST_TYPE (it->st_info) == STT_FUNC ) {	// function symbols
				...
				vxworks_register_symbol ( elf.mod_name, symbol_name, it->st_value );
#if 0
				//if ( !strcmp( "libc.so.6", elf.mod_name ) )
...
static bool
vxworks_read_elf_header (ELFInfo & elf)
{
	...
	elf.elf_header.e_type      = vxworks_byte_get (ehdr.e_type);
	elf.elf_header.e_version   = vxworks_byte_get (ehdr.e_version);
	elf.elf_header.e_entry     = vxworks_byte_get (ehdr.e_entry);
	elf.elf_header.e_phoff     = vxworks_byte_get (ehdr.e_phoff);
	elf.elf_header.e_shoff     = vxworks_byte_get (ehdr.e_shoff);		// section header offset
	elf.elf_header.e_flags     = vxworks_byte_get (ehdr.e_flags);
	elf.elf_header.e_ehsize    = vxworks_byte_get (ehdr.e_ehsize);
	elf.elf_header.e_phentsize = vxworks_byte_get (ehdr.e_phentsize);
	elf.elf_header.e_phnum     = vxworks_byte_get (ehdr.e_phnum);
	elf.elf_header.e_shentsize = vxworks_byte_get (ehdr.e_shentsize);	// section header size
	elf.elf_header.e_shnum     = vxworks_byte_get (ehdr.e_shnum);		// section header number
	elf.elf_header.e_shstrndx  = vxworks_byte_get (ehdr.e_shstrndx);
	...
	if ((elf.elf_header.e_shoff > 0 && elf.elf_header.e_shnum == 0) || !vxworks_read_elf_section_headers ( elf, 1 )) {
...
int vxworks_read_elf_info(CPUState *env, uint32_t cr3, const char * mod_name, target_ulong start_addr, uint64_t size) {
	...
	int res = (cr3 != -1U)
		&& vxworks_read_elf_header (elf)
		&& vxworks_process_section_header (elf)
		&& vxworks_process_section_headers (elf)
		&& vxworks_process_program_headers (elf)
		//&& process_dynamic_section (elf)
		&& vxworks_process_symbol_table (elf)
		;
