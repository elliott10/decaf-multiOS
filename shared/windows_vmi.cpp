...
/*static os_handle handle_funds[]= {
	{
		WINXP_SP2,
		&xp_offset,
		 &update_loaded_kernel_modulelist,
		&update_active_processlist,
		 &update_active_threadlist,
		&update_opened_filelist,
    &update_symbolslist,
	},
	{
		 WINXP_SP3,
...
static int update_active_processlist() {
	...
	//	cpu_memory_rw_debug(env, handle_table, (uint8_t *) &(pe->table_code), 4, 0);
		...
		//QLIST_INSERT_HEAD(&processlist, pe, loadedlist_entry);
		//monitor_printf(default_mon,"outside %d %d %08x %s\n", pe->pid, pe->parent_pid, pe->cr3, pe->name);
		if(findProcessByPidH(pe->pid)==0)//check if the process already exists else delete the process
		{   //monitor_printf(default_mon,"inside %d %d %08x %s\n", pe->pid, pe->parent_pid, pe->cr3, pe->name);
			new_proc=pe;
			addProcV(pe);
			tflag=1;
			newflag=1;
...
void message_m(uint32_t pid, uint32_t cr3, uint32_t base,module *pe){
	...
	monitor_printf(default_mon,"M %d %08x \"%s\" %08x %08x \"%s\"\n", pid, cr3, pe->name, base, pe->size,pe->fullname);
	sprintf(proc_mod_msg, "M %d %08x \"%s\" %08x %08x \"%s\"\n", pid, cr3, pe->name, base, pe->size,pe->fullname);
	//update_api_with_pe(cr3, pe, ((pid == 0 || pid == 4)? 0 : 1));
	/*if(!QLIST_EMPTY(&pe->apilist_head)){
		QLIST_FOREACH_SAFE(api, &pe->apilist_head, loadedlist_entry, next){
...
void update_kernel_modules(uint32_t cr3, target_ulong vaddr, uint32_t pid, struct cr3_info * cr3i)
{
	...
	while (curr_mod != 0 && curr_mod != psLM) {
		...
		cpu_memory_rw_debug(env, curr_mod + handle_funds[GuestOS_index].offset->DLLBASE_OFFSET, (uint8_t *) &(base), 4, 0); // dllbase  DLLBASE_OFFSET
		cpu_memory_rw_debug(env, curr_mod + handle_funds[GuestOS_index].offset->SIZE_OFFSET, (uint8_t *) &(curr_entry->size), 4, 0);           // dllsize  SIZE_OFFSET
		holder = readustr_with_cr3(curr_mod + handle_funds[GuestOS_index].offset->DLLNAME_OFFSET, 0,(curr_entry->name), env);
		readustr_with_cr3(curr_mod+0x24,0, curr_entry->fullname, env);
		if(!g_hash_table_lookup(cr3i->modules_tbl, (gpointer)base)) {
			//update_api_with_pe(cr3, curr_entry, 0);
			//message_m(pid, cr3,base, curr_entry);
			//g_hash_table_insert(cr3i->modules_tbl, (gpointer)(curr_entry->base), (gpointer)1);
			module *temp=findModule(curr_entry->fullname);
			if(temp==NULL)//if process exists then add module to it
						{
							procmod_insert_modinfoV(pid,base,curr_entry);
						    addModules(curr_entry);
							message_m(pid, cr3,base, curr_entry);
							g_hash_table_insert(cr3i->modules_tbl,(gpointer)(base), (gpointer)1);
						}
						else{
							procmod_insert_modinfoV(pid,base,temp);
							message_m(pid, cr3,base, temp);
							g_hash_table_insert(cr3i->modules_tbl,(gpointer)(base), (gpointer)1);
							delete curr_entry;
						}
		}
...
module* update_loaded_user_mods_with_peb(CPUState* env,uint32_t cr3,uint32_t peb, target_ulong vaddr,uint32_t pid, struct cr3_info* cr3i)
{
	uint32_t ldr, memlist, first_dll, curr_dll;
	module *curr_entry = NULL;
	...
	do {
		...
		memset(curr_entry, 0, sizeof(*curr_entry));
		uint32_t base;
		DECAF_memory_rw_with_cr3 (env,cr3, curr_dll+ 0x18, (void *) &(base), 4, 0);
		if(base == 0x0 && flag == 0){
			flag = 1;
		...
		readustr_with_cr3(curr_dll + 0x24, cr3, curr_entry->fullname, env);
		readustr_with_cr3(curr_dll + 0x2c, cr3, curr_entry->name, env);
		uint32_t modules = base;

		if(modules >0x00300000 && !g_hash_table_lookup(cr3i->modules_tbl, (gpointer)modules)){
...
target_ulong get_new_modules(CPUState* env, uint32_t cr3, target_ulong vaddr, struct cr3_info* cr3i){
	...
	if(cr3 == system_cr3) {
		...
		if(base !=0 && base == self){
			...
			uint32_t peb,ldr;
			cpu_memory_rw_debug(env,peb_addr,(uint8_t *)&peb, 4, 0);
			update_loaded_user_mods_with_peb(env,cr3, peb, vaddr, pid, cr3i);
		}
	}
	return 0;
}
...
void tlb_call_back(DECAF_Callback_Params *temp)
{
	...
	//monitor_printf(default_mon,"inside tlb \n");
	//CPUState *env = cpu_single_env ? cpu_single_env : first_cpu;
	//CPUState* env=temp->tx.env;
	//target_ulong vaddr=temp->tx.vaddr;
	    struct cr3_info* cr3i = NULL;
		...
		if(!DECAF_is_in_kernel()) {
				...
				if(!cr3i){ // new cr3'
					...
					if(system_proc == NULL) { //get the system proc first. This should be automatic.
							...
							update_kernel_modules(system_cr3, vaddr, procptr->pid, cr3i);
					...
					g_hash_table_insert(cr3_hashtable, (gpointer)cr3, (gpointer) cr3i);
					//addCr3info(cr3i);
					//insertvaddrincr3(cr3,vaddr);
					//cr3i->vaddr_tbl.insert(vaddr);
        ...
        if(!newflag) { // not a new cr3
				/*if(findvaddrincr3(cr3,vaddr)) {
						present_in_vtable++;
						goto done;
				}*/
				//g_hash_table_insert(cr3i->vaddr_tbl, (gpointer) vaddr, (gpointer)1);
...
void insn_end_cb_win(CPUState *env,uintptr_t insn_handle){
...
{
	//insn_end_cb_win(env,insn_handle);
